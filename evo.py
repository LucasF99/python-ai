        ...
        for i in range(len(self.pool)): # mutate function type
            for n in self.pool[i].net.neurons:
                ...
                if r <= 5*self.pool[i].get_mut_rate():
                    n.set_func(random.choice([nn.mean, nn.sigmoid, nn.tanh, nn.relu, nn.step, nn.step_neg]),*n.get_args())
        ...
        for i in range(len(self.pool)):
            ...
            while not dead:
                ...
                self.pool[i].update([c.get_pos()[0],c.get_pos()[1],c.get_speed()[0],c.get_speed()[1],
                                    rel_close_x, rel_close_y, mean_en_x, mean_en_y, en_x_stdev, en_y_stdev, width, height, c.get_accel()[0], c.get_accel()[1]]) # run net and set accel
